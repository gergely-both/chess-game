...
class Game:
    ...
    def promote_to(self, img_name, query_window):
        ...
        self.board_instance.canvas.delete(
            getattr(self.board_instance, str(self.promote_piece))
        )
        ...
        setattr(
            self.board_instance,
            str(new_piece),
            self.board_instance.canvas.create_image(
                self.promote_at.central_coordinates,
                image=getattr(self.board_instance, img_name),
            ),
        )
    ...
    def make_move(self, target_square):
        ...
        if self.attacked_figure:
            ...
            self.board_instance.canvas.delete(
                getattr(self.board_instance, str(self.attacked_figure))
            )
        elif self.enpass_enemy:
            del figures_squares_now[self.enpass_enemy]
            self.board_instance.canvas.delete(
                getattr(self.board_instance, str(self.enpass_enemy))
            )
        self.board_instance.canvas.move(
            getattr(self.board_instance, str(self.chosen_figure)), x, y
        )
        figures_squares_now[self.chosen_figure] = target_square

        if (self.chosen_figure.name == "white_pawn" and target.y == 8) or (
